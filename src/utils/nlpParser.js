...
async function parseReminderTextGPT(text) {
  ...
  const completion = await fetch(
    "https://bedrock-runtime.us-east-1.amazonaws.com/model/us.amazon.nova-pro-v1:0/converse",
    {
      ...
      body: JSON.stringify({
        ...
        system: [
          {
            text: `You are a chatbot for a reminder app. Check what reminder the client wants and return it as JSON like the following: 
        ${JSON.stringify({
          title: "Name of the task",
          priority:
            "LOW, MEDIUM, HIGH depending on the urgency/priority of the request",
          timeInfo:
            "Time the user wants the request to be fulfilled, in yyyy-mm-ddThh:mm:ss format, for reference, now is " +
            new Date().toISOString(),
          locationInfo:
            "A general place name where the request can be fulfilled",
        })}
  ...
  const val = JSON.parse(
    (await completion.json()).output.message.content[0].text
  );
